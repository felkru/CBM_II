...
def newton_verfahren(x0: np.ndarray, f_func: callable, grad_f_func: callable, hess_f_func: callable, M: int, x_optimized: np.ndarray = None, epsilon: float = None) -> tuple[np.ndarray, int | None]: #änderung: Signatur angepasst um M, x_optimized und epsilon zu erhalten. Rückgabe ist Tupel (xM, erster_konvergierter_schritt).
    ...
    x_history = [xk] #änderung: Array zum Speichern der Iterationspunkte
    ...
    for k in range(M):
        ...
        x_history.append(xk) # Aktuellen Punkt zur Historie hinzufügen
    ...
    # return xM, x_history #änderung: Rückgabe des Endpunkts und der Historie (falls benötigt)
    return xM, first_converged_step #änderung: Rückgabe des Endpunkts und des Schritts, bei dem epsilon unterschritten wurde.

# --- Ausführung für Teil d: M=2 Schritte ---
print("\n--- Ausführung für Teil d: M=2 Newton-Schritte ---")
M_part_d = 2

xM_result_d, _ = newton_verfahren(x0_initial, f_wrapper, grad_f_wrapper, hess_f_wrapper, M_part_d) #änderung: Wrapper verwenden, M=2, keine Konvergenzprüfung in diesem Aufruf
...
xM_result_e, first_converged_step = newton_verfahren(x0_initial, f_wrapper, grad_f_wrapper, hess_f_wrapper, M_max_part_e, x_optimized=x_optimized, epsilon=epsilon_part_e) #änderung: Aufruf mit x_optimized und epsilon
