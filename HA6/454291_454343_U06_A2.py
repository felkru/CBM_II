...
def plot_histogram_data(bin_edges, data_list, labels_list, title, xlabel="Value", ylabel="Counts", log_y=False):
    """
    Plots multiple histogram-like data sets using plt.stairs.
...
# --- a) Read data and plot measured signal ---
print("--- a) Reading data and plotting measured signal ---")

# Load bin edges
# bins.txt has 21 lines for 20 bins
bins_file = data_path / "bins.txt"
bin_edges = np.loadtxt(bins_file)
print(f"Loaded {len(bin_edges) - 1} bins from {bins_file}")

# Load folded data (measured counts)
# FoldedData.txt has 20 lines (counts for each bin)
folded_data_file = data_path / "FoldedData.txt"
...
# convolution_matrix.csv is 20x20
# Note: Faltungsmatrix.py should be run once to generate this, or ensure it exists.
matrix_file = data_path / "convolution_matrix.csv"
...
# --- c) Unfold data using Likelihood method (Richardson-Lucy like) ---
...
if np.sum(c_likelihood) <= 0:  # Handle case where d_measured sums to 0 or less
...
for iteration in range(num_iterations_likelihood):
    ...
    # Element-wise division, add epsilon to denominator for stability
    ratio = d_measured / (Rc + epsilon)
    correction_factor = R_matrix.T @ ratio  # R_ij in formula means R[i,j] (row i, col j)
    # This corresponds to standard matrix notation.
...
plot_histogram_data(bin_edges, [d_measured, c_likelihood],
                    ["Measured Data (d)", "Unfolded (Likelihood Method)"],
                    f"Unfolding via Likelihood Method ({num_iterations_likelihood} iter.)",
                    "Bin Index", "Counts")

# --- d) Unfold data using Tikhonov Regularization ---
...
plot_histogram_data(bin_edges, c_tikhonov_results, plot_labels_tikhonov,
                    ...
                    "Bin Index", "Counts", log_y=True)  # Log y can be helpful here
...
# --- e) Comparison with true distribution ---
...
plot_histogram_data(bin_edges,
                    ...
                    "Bin Value (from bins.txt)", "Counts", log_y=True)
